  ...
  class Evaluator
    ...
    def evaluate heuristic_class, dataset, csv: false
      ...
      results = partitions.zip(partitions.map(&:reverse)).flatten(1).map.each_with_index do |(train, test), index|
        ...
        finish = Time.now
        evaluation = heuristic.classifier.class.new(CONFIG.knn[:num_neighbors], test.instances, test.classes, test.numeric_attrs, Random.new(@seeds[index])).fitness_for(solution)
